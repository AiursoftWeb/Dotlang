    ...
    MarkdownShredder shredder)
{
    private const string Prompt =
        ...
        I am a programmer working on a software localization project. I need to localize some Markdown content into the target language {LANG}. Unfortunately, I don't understand the {LANG} language or the local culture. Therefore, I need you to act as an experienced translator who can accurately, fluently, and carefully choose words to translate the content into the {LANG} language.

        The Markdown content you need to translate is:
        ```
        {CONTENT}
        ```
        ...
        1. **DO NOT translate inline code.** Text wrapped in backticks (`) is source code, a command, a package name, or a file name. Output it exactly as-is, including the backticks. Examples: `command`, `package-name`, `filename.ext`, `/etc/config`, `v2.0.1`

        2. **DO NOT translate these items.** Output them exactly as-is:
           - Package names and software names (e.g. anduinos-xxx, GNOME, Wayland, Ubuntu)
           - File paths (e.g. /etc/..., /usr/share/...)
           - URLs and hyperlinks — preserve both the URL and the link syntax
           - Version numbers (v2.0.1, 1.4.2, etc.)
           - CLI commands (sudo apt install, dotnet run, etc.)
           - Brand names, trademarks, product names
           - Protocol and technology names (ARM64, UEFI, Zram, Zswap, Secure Boot, LZ4, Wine, Proton)
           - Variable names, configuration keys, environment variable names

        3. **Preserve ALL Markdown formatting exactly.** Keep **bold**, *italic*, `inline code`, # headers, [link text](url), bullet markers (* - + 1.), table rows (|...|), admonitions (!!! note, ??? warning), blockquotes (>), horizontal rules (---), and all other Markdown syntax exactly as they appear. Only translate the human-readable natural-language text within them.
        ...
        [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        logger.LogInformation(
            @"Calling Ollama stream to translate chunk ({Length} chars), Instance: ""{instance}"", Model: ""{model}""",
            trimmedSource.Length, options.Value.OllamaInstance, options.Value.OllamaModel);
        ...
        CancellationToken cancellationToken)
    {
        ...
        logger.LogInformation(
            @"Calling Ollama to translate chunk ({Length} chars), Instance: ""{instance}"", Model: ""{model}""",
            trimmedSource.Length, options.Value.OllamaInstance, options.Value.OllamaModel);
        ...
        logger.LogInformation(@"Ollama translation result ({Length} chars)", aiResponseRaw.Length);
